...
struct WeeklyInvoiceView: View {
    ...
    private func drawDetailPage(cgContext cg: CGContext, job: Job) {
        ...
        "CASE REASON: PALFIELD".draw(at: CGPoint(x: 50, y: y), withAttributes: [.font: header, .foregroundColor: UIColor.black])
        y += 40

        // PALFIELD table header with black background
        cg.setFillColor(UIColor.black.cgColor)
        cg.fill(CGRect(x: 50, y: y - 5, width: 512, height: 25))

        "PALFIELD".draw(at: CGPoint(x: 55, y: y), withAttributes: [.font: header, .foregroundColor: UIColor.white])
        ...
        line("Flat panel palfield same stud", qty: job.flatPanelStud, price: settings.priceForFlatPanelStud())
        line("Flat panel palfield same wall", qty: job.flatPanelWall, price: settings.priceForFlatPanelWall())
        line("Flat panel palfield remote", qty: job.flatPanelRemote, price: settings.priceForFlatPanelRemote())
