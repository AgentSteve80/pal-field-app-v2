...
class PalFieldAssistant {
    static let shared = PalFieldAssistant()
    ...
    func answer(question: String, emails: [CachedEmail] = []) -> AssistantResponse {
        ...
        if q.contains("widget") {
            return withSearchResults(appResponse("Widgets", "Add Pal Field widgets to your home screen to see weekly earnings and job count at a glance. Widgets update automatically when you add or edit jobs."))
        }
